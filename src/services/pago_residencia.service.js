...
const getAllPagos = async (token, desde, hasta) => {
    ...
    const response = await axios.get(`${API_URL}/pagos_residencia`, config);
...
const getPagoByStudent = async (id, token, desde, hasta) => {
    ...
    const response = await axios.get(`${API_URL}/pagos_residencia/estudiante/${id}`, config);
    return response.data;
}
...
const updateEstadoPago = async (data, token) => {
    ...
    const response = await axios.put(`${API_URL}/pagos_residencia/estado/${data._id}`, data, config);
    if (response.status === 200 || response.status === 201) {
